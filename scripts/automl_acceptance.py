...
def inspect(root: Path) -> dict:
    errors: list[str] = []
    path = root / "compliance" / "AUTOML_ACCEPTANCE.json"
    if not path.is_file():
        return {"status": "fail", "errors": ["Missing compliance/AUTOML_ACCEPTANCE.json"]}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        return {"status": "fail", "errors": [f"Invalid AUTOML_ACCEPTANCE.json: {exc}"]}
    version = (root / "VERSION").read_text(encoding="utf-8").strip()
    if payload.get("product_version") != version:
        errors.append(f"AutoML manifest version={payload.get('product_version')!r}, VERSION={version!r}")
    items = payload.get("items") or []
    if [item.get("id") for item in items] != EXPECTED_IDS:
        errors.append("AutoML acceptance items do not match the v2.50 contract")
    for item in items:
...
def run_runtime_tests(root: Path) -> tuple[int, str]:
    command = [sys.executable, "-m", "pytest", "-q", "tests/test_automl_v2500.py", "tests/test_frontend_automl_v2500.py"]
    env = dict(os.environ)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate DataVision v2.50 AutoML completion.")
    ...
    if args.check and result["status"] == "pass":
        ...
        if code:
            result["status"] = "fail"
            result["errors"].append("Executable AutoML acceptance tests failed")
    elif args.check:
        result["runtime_test"] = "not_run"
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        print(f"AutoML acceptance: {result['status'].upper()} ({result.get('items', 0)}/8 items)")
