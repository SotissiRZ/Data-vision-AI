...
## v2.38.0 — Semantic Memory Actions

Aucune migration manuelle de base n'est nécessaire. La mémoire projet existante reste compatible. Les nouvelles recettes créées à partir de v2.38 conservent davantage de paramètres compacts (features/variables, type de régression, options de test et rapport) afin de permettre une relance gouvernée.
...
Le préflight doit afficher `VERSION = 2.38.0` et `cryptography==46.0.5`.


## v2.39.0 — Repository Cleanup

Aucune migration de base de données n'est nécessaire.

Les fichiers historiques précédemment présents à la racine ont été déplacés :

- `MERGE_MANIFEST_V*.json` → `docs/history/manifests/`
- `MIGRATION_FROM_V212.md` → `docs/history/migrations/MIGRATION_FROM_V212.md`

Les scripts Windows opérationnels restent à la racine.
