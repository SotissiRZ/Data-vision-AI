...
La v2.2 transforme les policies de v2.1 en véritable boundary de données. Une session Entreprise active ne doit jamais pouvoir appeler une ancienne route analytique et récupérer le dataset brut par accident.
...
`storage.load_dataframe(dataset_id)` est désormais le point d'enforcement universel. En mode Entreprise il vérifie :
...
Le mode local demeure compatible lorsque aucun header Entreprise n'est présent.
...
L'accès à une lignée est accordé si une version de cette même racine est liée au workspace. Les versions dérivées créées sous contexte Entreprise sont automatiquement liées au workspace.
