...
La v2.12 renforce la frontière Entreprise sur trois axes : sessions révocables côté serveur, SSO OIDC moderne et coffre de secrets versionné. Elle complète l'authentification locale de v2.1 sans supprimer le mode local ni les rôles/workspaces existants.

## Sessions Entreprise persistantes
...
Le refresh token brut n'est pas stocké : seul son SHA-256 est conservé. L'access token contient un `sid` et chaque requête Entreprise vérifie que la session correspondante n'est ni révoquée ni expirée. Un utilisateur peut lister ses sessions, en révoquer une ou fermer toutes ses autres sessions.
...
La page de connexion Entreprise affiche automatiquement les fournisseurs SSO actifs.
