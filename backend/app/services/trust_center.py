...
def trust_center(dataset_id: str, df: pd.DataFrame) -> dict[str, Any]:
    ...
    if metric_count:
        semantics_score = round(35 + 35 * certified / metric_count + 30 * described / metric_count)
    else:
        semantics_score = 20
    semantics_score = int(max(0, min(100, semantics_score)))
    ...
    checks = [
        ...
        {"area": "Sémantique", "status": "pass" if semantics_score >= 80 else "warn", "score": semantics_score, "evidence": f"{certified}/{metric_count or 0} métriques certifiées"},
