...
function OperationalIntelligenceCenter({result,setError,setView}:{result:AnyObj|null;setError:(s:string)=>void;setView:(v:View)=>void}){
  ...
  async function refresh(t=token,ws=workspaceId){
    ...
    try{
      const [o,s,u,e,ev]=await Promise.all([getOperationalOverview(t,ws,hours),getSREStatus(t,ws,hours),getOperationalUsage(t,ws,720),getOperationalTelemetry(t,ws,hours,80),getEvaluationSuites(t,ws)]);
      setOverview(o);setSre(s);setUsage(u);setEvents(e.events??[]);setSuites(ev.suites??[]);
  ...
  return <div className="page operational-page">
    <div className="page-title"><div><span className="eyebrow">OPERATIONAL INTELLIGENCE · SRE V2.60</span><h1>Observabilité & Evaluation</h1><p>SLO, budgets d’erreur, sauvegardes, restore drills, file de jobs et tests de non-régression dans un même cockpit SRE.</p></div><div className="operations-head-actions"><label>Workspace<select value={workspaceId} onChange={e=>changeWorkspace(e.target.value)}>{(session.workspaces??[]).map((w:AnyObj)=><option key={w.id} value={w.id}>{w.name} · {w.role}</option>)}</select></label><label>Fenêtre<select value={hours} onChange={e=>setHours(Number(e.target.value))}><option value={6}>6 h</option><option value={24}>24 h</option><option value={168}>7 j</option><option value={720}>30 j</option></select></label><button className="secondary-btn" onClick={()=>refresh()} disabled={busy}>↻ Actualiser</button></div></div>
