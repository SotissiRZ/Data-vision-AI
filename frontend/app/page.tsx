...
useEffect(()=>{
  ...
  async function doAutoML(config: { task: 'auto'|'classification'|'regression'|'clustering'; features?: string[]; primary_metric: string; cv_folds: number; tune: boolean; max_candidates: number; split_strategy?: 'auto'|'random'|'temporal'; time_column?: string|null }) {
    if (!result || (config.task !== 'clustering' && !target)) return;
    setAutomlRunning(true); setError('');
    try {
      const trained = await runAutoML(result.dataset.id, { target: config.task === 'clustering' ? null : target, ...config });
      setModel(trained);
...
function ModelView({ result, target, setTarget, algorithm, setAlgorithm, training, automlRunning, doTrain, doAutoML, model, setError }: { result: AnyObj | null; target: string; setTarget:(v:string)=>void; algorithm:string; setAlgorithm:(v:string)=>void; training:boolean; automlRunning:boolean; doTrain:()=>void; doAutoML:(c:{task:'auto'|'classification'|'regression'|'clustering';features?:string[];primary_metric:string;cv_folds:number;tune:boolean;max_candidates:number;split_strategy?:'auto'|'random'|'temporal';time_column?:string|null})=>void; model:AnyObj|null; setError:(s:string)=>void }) {
  const [task,setTask]=useState<'auto'|'classification'|'regression'|'clustering'>('auto');
  ...
  useEffect(()=>{
    if(task==='clustering' && !['auto','silhouette','calinski_harabasz','davies_bouldin'].includes(metric)) setMetric('auto');
    if(task!=='clustering' && ['silhouette','calinski_harabasz','davies_bouldin'].includes(metric)) setMetric('auto');
  },[task]);
  ...
  async function runBenchmarkOnly(){
    ...
    try{
      setBenchmarkOnly(await runModelBenchmark(result!.dataset.id,{
        target:clustering?null:target,
        task,
        features:clustering?clusterFeatures:undefined,
        primary_metric:metric,
        cv_folds:folds,
        max_candidates:maxCandidates,
        split_strategy:clustering?'auto':splitStrategy,
        time_column:!clustering&&splitStrategy!=='random'&&timeColumn?timeColumn:null,
      }));
    }catch(e:unknown){setError(e instanceof Error?e.message:String(e));}
  ...
  async function auditSafety(){
    if(!clustering&&!target){setError('Sélectionnez une variable cible avant l’audit ML Safety.');return;}
    if(clustering&&clusterFeatures.length<2){setError('Sélectionnez au moins deux variables numériques pour le clustering.');return;}
    setSafetyBusy(true);setError('');
    try{
      setSafetyAudit(await runMLSafetyAudit(result!.dataset.id,{
        target:clustering?null:target,task,features:clustering?clusterFeatures:undefined,primary_metric:metric,
        split_strategy:clustering?'auto':splitStrategy,time_column:!clustering&&splitStrategy!=='random'&&timeColumn?timeColumn:null,
      }));
    }catch(e:unknown){setError(e instanceof Error?e.message:String(e));}
  ...
  function launchAutoML(){
    if(clustering&&clusterFeatures.length<2){setError('Sélectionnez au moins deux variables numériques pour le clustering.');return;}
    void doAutoML({task,features:clustering?clusterFeatures:undefined,primary_metric:metric,cv_folds:folds,tune,max_candidates:maxCandidates,split_strategy:clustering?'auto':splitStrategy,time_column:!clustering&&splitStrategy!=='random'&&timeColumn?timeColumn:null});
  }

  return <div className="page">
    <div className="page-title">
      <div><span className="eyebrow">MACHINE LEARNING · AUTOML · EXPERIMENTS</span><h1>Modélisation professionnelle</h1><p>Classification, régression et clustering avec benchmark, ML Safety exécutable, split gouverné, contrôle du leakage et historique d’expériences.</p></div>
      <span className="module-state implemented">ML Safety v2.51</span>
    </div>
    ...
    <div className="two-col model-layout">
      <Panel title="AutoML gouverné">
        <div className="stack-form">
          <label>Tâche<select value={task} onChange={e=>setTask(e.target.value as 'auto'|'classification'|'regression'|'clustering')}><option value="auto">Auto — classification/régression</option><option value="classification">Classification</option><option value="regression">Régression</option><option value="clustering">Clustering non supervisé</option></select></label>
          {!clustering&&<label>Variable cible<select value={target} onChange={e=>setTarget(e.target.value)}>{result.profile.columns.filter((c:AnyObj)=>!isLikelyIdentifier(c,Number(result.profile.rows??0))).map((c:AnyObj)=><option key={c.name}>{c.name}</option>)}</select></label>}
          {clustering&&<div className="automl-feature-picker"><b>Variables de clustering</b><small>Variables numériques uniquement. Les identifiants probables sont exclus.</small><div>{numericColumns.map((c:AnyObj)=><label key={c.name}><input type="checkbox" checked={clusterFeatures.includes(c.name)} onChange={()=>toggleClusterFeature(c.name)}/><span>{c.name}</span></label>)}</div></div>}
          <div className="form-row">
            ...
            {!clustering&&<label>CV<input type="number" min={2} max={10} value={folds} onChange={e=>setFolds(Number(e.target.value))}/></label>}
          ...
          <small className="help-text">{clustering?'Le clustering est sélectionné sur métriques internes ; aucune cible ni pseudo-test supervisé n’est utilisé.':'Le jeu de test final reste isolé jusqu’à l’évaluation du modèle sélectionné.'}</small>
        </div>
      </Panel>
      ...
      <Panel title="Entraînement manuel supervisé">
        <div className="stack-form">
          ...
          <button className="secondary-btn real-button" disabled={training||clustering} onClick={doTrain}>{training?'Entraînement…':'Entraîner un modèle précis'}</button>
    ...
    {model&&<>
      <div className="metrics six"><Stat label="Tâche" value={model.task}/><Stat label="Algorithme retenu" value={model.algorithm}/><Stat label="Train" value={model.rows_train}/><Stat label="Validation" value={model.rows_validation??'—'}/><Stat label={model.task==='clustering'?'Test supervisé':'Test final'} value={model.rows_test}/><Stat label="Métrique" value={model.primary_metric??model.model_card?.primary_metric??'auto'}/></div>
      <Panel title={model.task==='clustering'?'Métriques internes de clustering':'Métriques — test final'} action={<code>{model.experiment_id?.slice(0,8)??model.model_id?.slice(0,8)}…</code>}><div className="metric-results">{Object.entries(model.metrics??{}).map(([k,v])=><span key={k}><small>{k}</small><b>{formatNumber(v)}</b></span>)}</div>{model.selection_rationale&&<div className="validation-line"><b>Sélection :</b><span>{model.selection_rationale}</span></div>}</Panel>
      {benchmark.length>0&&<Panel title="Leaderboard AutoML"><SimpleTable rows={benchmark} columns={model.task==='clustering'?[["rank","Rang"],["label","Candidat"],["validation_score","Score"],["k","k"],["status","Statut"]]:[["rank","Rang"],["label","Modèle"],["validation_score","Validation"],["cv_mean","CV moyenne"],["cv_std","CV σ"]]}/></Panel>}
...
function InsightEngineView({result,setError,setView}:{result:AnyObj|null;setError:(s:string)=>void;setView:(v:View)=>void}){
  ...
  return <div className="page insight-engine-page">
    ...
    {feed?.limitations?.length>0&&<Panel title="Limites d'interprétation"><div className="warning-box">{feed.limitations.map((x:string)=><p key={x}>• {x}</p>)}</div></Panel>}
  </div>;
}
...
function ReportView({ result, setError }: { result:AnyObj|null; setError:(s:string)=>void }) {
  ...
  async function create(){setBusy(true);setError('');try{const parsed=customBlocks.map(b=>{let cfg:AnyObj={};try{cfg=JSON.parse(b.payload||'{}');}catch{throw new Error(`Configuration JSON invalide pour le bloc ${b.title}`);}return {id:b.id,type:b.type,title:b.title,...cfg};});const order=[...sections.map(x=>`section:${x}`),...customBlocks.map(x=>x.id)];const r=await createReport(result!.dataset.id,{title,subtitle,author,organization,template,sections,analysis_session_id:analysisId||null,visualization_ids:visualIds,auto_story:autoStory,auto_visualizations:autoVisuals,max_visualizations:maxVisuals,custom_blocks:parsed,block_order:order});setCreated(r);setValidation(await validateReport(result!.dataset.id,r.id));await refresh();}catch(e:unknown){setError(e instanceof Error?e.message:String(e));}finally{setBusy(false);}}
...
function OperationalIntelligenceCenter({result,setError,setView}:{result:AnyObj|null;setError:(s:string)=>void;setView:(v:View)=>void}){
  ...
  async function refresh(t=token,ws=workspaceId){
    ...
    try{
      const [o,u,e,ev]=await Promise.all([getOperationalOverview(t,ws,hours),getOperationalUsage(t,ws,720),getOperationalTelemetry(t,ws,hours,80),getEvaluationSuites(t,ws)]);
      setOverview(o);setUsage(u);setEvents(e.events??[]);setSuites(ev.suites??[]);
  ...
  return <div className="page operational-page">
    <div className="page-title"><div><span className="eyebrow">OPERATIONAL INTELLIGENCE · V2.9</span><h1>Observabilité & Evaluation</h1><p>SLO, latence, fiabilité des jobs, télémétrie d’usage et tests de non-régression de l’AI Analyst dans un même cockpit.</p></div><div className="operations-head-actions"><label>Workspace<select value={workspaceId} onChange={e=>changeWorkspace(e.target.value)}>{(session.workspaces??[]).map((w:AnyObj)=><option key={w.id} value={w.id}>{w.name} · {w.role}</option>)}</select></label><label>Fenêtre<select value={hours} onChange={e=>setHours(Number(e.target.value))}><option value={6}>6 h</option><option value={24}>24 h</option><option value={168}>7 j</option><option value={720}>30 j</option></select></label><button className="secondary-btn" onClick={()=>refresh()} disabled={busy}>↻ Actualiser</button></div></div>
