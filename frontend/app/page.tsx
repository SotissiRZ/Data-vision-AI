...
import {
  ...
  runNaturalLanguageQuery, getAIHistory, getAIHistoryItem, getReports, createReport, downloadReport,
} from '../lib/api';
...
function isLikelyIdentifier(c: AnyObj, rows = 0) { const n=String(c?.name??'').toLowerCase(); const byName=/^(id|index|row|record|patient_id|customer_id|user_id)$/.test(n)||/_id$/.test(n); const unique=Number(c?.unique??0); return byName || (rows>20 && unique/Math.max(rows,1)>.98); }
...
function PcaView({ result, setError }: { result:AnyObj|null; setError:(s:string)=>void }) {
  const nums=result?.profile?.columns?.filter(isNumeric)??[]; const meaningful=nums.filter((c:AnyObj)=>!isLikelyIdentifier(c,Number(result?.profile?.rows??0))); const [selected,setSelected]=useState<string[]>([]); const [scale,setScale]=useState(true); const [analysis,setAnalysis]=useState<AnyObj|null>(null); const [busy,setBusy]=useState(false);
...
function ClusterView({ result, setError }: { result:AnyObj|null; setError:(s:string)=>void }) {
  const nums=result?.profile?.columns?.filter(isNumeric)??[]; const meaningful=nums.filter((c:AnyObj)=>!isLikelyIdentifier(c,Number(result?.profile?.rows??0))); const [selected,setSelected]=useState<string[]>([]); const [k,setK]=useState(3); const [analysis,setAnalysis]=useState<AnyObj|null>(null); const [busy,setBusy]=useState(false);
  useEffect(()=>{if(result){setSelected((meaningful.length?meaningful:nums).slice(0,Math.min(5,(meaningful.length?meaningful:nums).length)).map((c:AnyObj)=>c.name));setAnalysis(null);}},[result?.dataset?.id]);
...
function AnomalyView({ result, setError }: { result:AnyObj|null; setError:(s:string)=>void }) {
  const allNumeric=(result?.profile?.columns??[]).filter(isNumeric); const numeric=allNumeric.filter((c:AnyObj)=>!isLikelyIdentifier(c,Number(result?.profile?.rows??0))); const [selected,setSelected]=useState<string[]>([]); const [method,setMethod]=useState('auto'); const [contamination,setContamination]=useState(0.05); const [out,setOut]=useState<AnyObj|null>(null); const [busy,setBusy]=useState(false);
...
function ReportView({ result, setError }: { result:AnyObj|null; setError:(s:string)=>void }) {
  const [title,setTitle]=useState('Rapport DataVision'); const [sections,setSections]=useState<string[]>(['overview','quality','descriptive','ai_analysis','methodology','provenance']);
  const [analyses,setAnalyses]=useState<AnyObj[]>([]); const [analysisId,setAnalysisId]=useState(''); const [reports,setReports]=useState<AnyObj[]>([]); const [created,setCreated]=useState<AnyObj|null>(null); const [busy,setBusy]=useState(false);
  const choices=[['overview','Vue d’ensemble'],['quality','Qualité'],['descriptive','Statistiques descriptives'],['ai_analysis','AI Analyst'],['methodology','Méthodologie'],['provenance','Provenance']];
  async function refresh(){if(!result)return; try{const [a,r]=await Promise.all([getAIHistory(result.dataset.id),getReports(result.dataset.id)]);setAnalyses(a.analyses??[]);setReports(r.reports??[]);if(!analysisId&&a.analyses?.[0]?.session_id)setAnalysisId(a.analyses[0].session_id);}catch(e:unknown){setError(e instanceof Error?e.message:String(e));}}
  useEffect(()=>{setCreated(null);refresh();},[result?.dataset?.id]);
  if(!result)return <EmptyState title="Rapports" text="Chargez un dataset pour construire un rapport reproductible."/>;
  async function create(){setBusy(true);setError('');try{const r=await createReport(result!.dataset.id,{title,sections,analysis_session_id:analysisId||null});setCreated(r);await refresh();}catch(e:unknown){setError(e instanceof Error?e.message:String(e));}finally{setBusy(false);}}
  ...
  return <div className="page"><div className="page-title"><div><span className="eyebrow">REPRODUCIBLE REPORT BUILDER</span><h1>Rapports</h1><p>Construisez un rapport verrouillé sur la version active du dataset et, si souhaité, sur une session AI Analyst précise.</p></div><span className="module-state implemented">v1.0</span></div>
    <div className="two-col"><Panel title="Composer le rapport"><div className="stack-form"><label>Titre<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Session AI Analyst<select value={analysisId} onChange={e=>setAnalysisId(e.target.value)}><option value="">Aucune</option>{analyses.map((a:AnyObj)=><option key={a.session_id} value={a.session_id}>{a.intent} · {a.question.slice(0,70)}</option>)}</select></label><div className="report-sections">{choices.map(([key,label])=><label key={key}><input type="checkbox" checked={sections.includes(key)} onChange={e=>setSections(e.target.checked?[...sections,key]:sections.filter(x=>x!==key))}/>{label}</label>)}</div><RunButton busy={busy} label="Créer le rapport" busyLabel="Génération…" onClick={create}/><small className="help-text">Dataset verrouillé : {result.dataset.name} · version {result.dataset.version??1}</small></div></Panel>
      <Panel title="Reproductibilité"><ul className="guardrails compact"><li>✓ Version exacte du dataset conservée</li><li>✓ Provenance et timestamp intégrés</li><li>✓ Résultats numériques issus des moteurs exécutables</li><li>✓ Session AI Analyst optionnellement verrouillée</li><li>✓ Exports PDF / DOCX / HTML / Markdown</li></ul></Panel></div>
    {created&&<Panel title="Rapport généré"><div className="report-created"><div><b>{created.title}</b><span>ID {created.id}</span></div><div className="button-row"><button className="secondary-btn" onClick={()=>dl(created.id,'pdf')}>PDF</button><button className="secondary-btn" onClick={()=>dl(created.id,'docx')}>DOCX</button><button className="secondary-btn" onClick={()=>dl(created.id,'html')}>HTML</button><button className="secondary-btn" onClick={()=>dl(created.id,'md')}>Markdown</button></div></div></Panel>}
    <Panel title="Historique des rapports" action={<span className="quiet">{reports.length} rapport(s)</span>}>{reports.length?<div className="report-list">{reports.map((r:AnyObj)=><div key={r.id}><div><b>{r.title}</b><small>v{r.dataset_version} · {r.created_at?new Date(r.created_at).toLocaleString('fr-FR'):''} · {(r.sections??[]).join(', ')}</small></div><div className="button-row"><button onClick={()=>dl(r.id,'pdf')}>PDF</button><button onClick={()=>dl(r.id,'docx')}>DOCX</button><button onClick={()=>dl(r.id,'html')}>HTML</button><button onClick={()=>dl(r.id,'md')}>MD</button></div></div>)}</div>:<div className="quiet-empty">Aucun rapport généré pour cette version.</div>}</Panel>
  </div>;
}
