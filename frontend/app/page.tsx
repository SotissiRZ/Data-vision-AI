...
function XaiView({ result, model, setError }: { result:AnyObj|null; model:AnyObj|null; setError:(s:string)=>void }) {
  ...
  async function runCf(){setXaiBusy('cf');setError('');try{const row=JSON.parse(rowText) as Record<string, unknown>;const payload:Parameters<typeof runModelCounterfactuals>[1]={row,max_changes:2,max_results:5};if(activeModel.task==='regression'){if(desiredValue.trim())payload.desired_value=Number(desiredValue);else payload.direction='increase';}setCf(await runModelCounterfactuals(activeModel.model_id,payload));}catch(e:unknown){setError(e instanceof Error?e.message:String(e));}finally{setXaiBusy('');}}
...
function SemanticStudio({result,setError}:{result:AnyObj|null;setError:(s:string)=>void}){
  ...
  const [busy,setBusy]=useState(false);
  const [tab,setTab]=useState<'model'|'metrics'|'dimensions'|'hierarchies'|'query'>('model');
  ...
  const metrics=semantic.metrics??[],dimensions=semantic.dimensions??[],tables=semantic.tables??[],relationships=semantic.relationships??[],hierarchies=semantic.hierarchies??[];
  ...
  return <div className="page semantic-page semantic-v2-page">
    ...
    <nav className="semantic-tabs">{([['model','Modèle & relations'],['metrics','Métriques'],['dimensions','Dimensions'],['hierarchies','Hiérarchies'],['query','Query Lab']] as const).map(([k,l])=><button key={k} className={tab===k?'active':''} onClick={()=>setTab(k)}>{l}</button>)}</nav>
